...
public class IcoTextView extends TextView {
    ...
    public IcoTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public IcoTextView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(attrs);
    }

    public void setTextTypface(String name) {
        Typeface tf = Typeface.createFromAsset(getContext().getAssets(), String.format("fonts/%s.ttf", name));
        setTypeface(tf);
    }
    ...
    public void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.IcoTextView);

            String typefaceName = a.getString(R.styleable.IcoTextView_typeface);
            if (typefaceName != null) {
                Typeface tf = Typeface.createFromAsset(getContext().getAssets(), String.format("fonts/%s.ttf", typefaceName));
                setTypeface(tf);
            }

            if (a.hasValue(R.styleable.IcoTextView_foreground)) {
                Drawable foreground = a.getDrawable(R.styleable.IcoTextView_foreground);
                if (foreground != null) {
                    this.setForegroundDrawable(foreground);
                } else {
                    this.setTextColor(a.getColor(R.styleable.IcoTextView_foreground, 0xff000000));
                }
            }


            if (a.hasValue(R.styleable.IcoTextView_innerShadowColor)) {
                this.addInnerShadow(a.getFloat(R.styleable.IcoTextView_innerShadowRadius, 0),
                        a.getFloat(R.styleable.IcoTextView_innerShadowDx, 0),
                        a.getFloat(R.styleable.IcoTextView_innerShadowDy, 0),
                        a.getColor(R.styleable.IcoTextView_innerShadowColor, 0xff000000));
            }

            if (a.hasValue(R.styleable.IcoTextView_outerShadowColor)) {
                this.addOuterShadow(a.getFloat(R.styleable.IcoTextView_outerShadowRadius, 0),
                        a.getFloat(R.styleable.IcoTextView_outerShadowDx, 0),
                        a.getFloat(R.styleable.IcoTextView_outerShadowDy, 0),
                        a.getColor(R.styleable.IcoTextView_outerShadowColor, 0xff000000));
            }

            if (a.hasValue(R.styleable.IcoTextView_strokeColor)) {
                float strokeWidth = a.getFloat(R.styleable.IcoTextView_strokeWidth, 1);
                int strokeColor = a.getColor(R.styleable.IcoTextView_strokeColor, 0xff000000);
                float strokeMiter = a.getFloat(R.styleable.IcoTextView_strokeMiter, 10);
                Join strokeJoin = null;
                switch (a.getInt(R.styleable.IcoTextView_strokeJoinStyle, 0)) {
                    case (0):
